...
class SoundsCog(commands.Cog, name="Make the bot make noise."):
    ...
    async def info_error(self, ctx, error):
        if "message" in dir(error.original):
            ...
            logging.info(error)
            await ctx.send(
                "An error has occured. Sorry can't be less secure. ask the developer."
            )
...
class MiscCog(commands.Cog, name="Miscellaneous functions"):
    ...
    async def simpwatch(self, message):
        if mesage.author == self.bot:
            return
             
        if message.channel.name != "simp":
            return

        else:
            member = message.guild.get_member(216321712067706881)
            await member.send(message)

    @commands.Cog.listener(name="on_member_update")
    async def bigcockalarm(self, before, after):
        if 321054194054201344 == before._user.id and before.nick != after.nick:
            await after.guild.get_member(321054194054201344).edit(
                nick=config.BOTNAME)
        limp_dicks = []

        if 719058425241272411 in [
                role.id for role in before.roles
        ] and 719058425241272411 not in [role.id for role in after.roles]:
        ...
        elif 719057975645306930 in [
                role.id for role in after.roles
        ] and 719057975645306930 not in [role.id for role in before.roles]:
            if after._user.id in limp_dicks:
                await after.remove_roles(
                    after.guild.get_role(719057975645306930))
    ...
    async def ascii(self, ctx, *args):
        ...
        try:
            flags = {
                "--more-detail": bool(flags["--more-detail"]),
                "--cols": int(flags["--cols"]),
        ...
        art = Asciiifier(picture, flags["--more-detail"], flags["--cols"],
                         flags["--scale"])
    ...
    async def uhoh(self, ctx, youtube_link):
        ...
        if hour - 6 >= 2 and hour - 6 <= 8:
            await ctx.send(
                "Hour is too unholy to annoy josh. Let me sleep please")
            return
        ...
        if pattern.match(youtube_link):
            ...
            with youtube_dl.YoutubeDL() as ydl:
                length = ydl.extract_info(youtube_link,
                                          download=False).get("duration")
                if length > 31 and not josh:
                    await ctx.send(f"Too Long, length was {length}")
                    return
            message = await ctx.send("Sending message")
            resp = requests.post(
                f"http://192.168.86.69/alert?url={youtube_link}&is_josh={josh}&password=L5FwBEZNxBE"
            )

            logging_channel = discord.utils.get(ctx.guild.channels,
                                                name="bot-logs")
            await logging_channel.send(
                f"{ctx.author} sent a message\n{youtube_link}")
    ...
    async def purge(self, ctx, member: discord.Member, **flags):
        ...
        if flags["previous"] and flags["message"]:
            await ctx.send(
                "Previous and Message are mutually exclusive. Use one or the other dipshit."
            )
            return
        ...
        if flags["previous"] and not previous_message:
            await ctx.send(
                f"Could not find a message by {member.name}, try running the command with with a higher --limit."
            )
        count = 0
        async for message in ctx.history(limit=flags["limit"]):
            if message.author == member:
